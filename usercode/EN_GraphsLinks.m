function varargout = EN_GraphsLinks(varargin)
% EN_GraphsLinks M-file for EN_GraphsLinks.fig
%      EN_GraphsLinks, by itself, creates a new EN_GraphsLinks or raises the existing
%      singleton*.
%
%      H = EN_GraphsLinks returns the handle to a new EN_GraphsLinks or the handle to
%      the existing singleton*.
%
%      EN_GraphsLinks('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in EN_GraphsLinks.M with the given input arguments.
%
%      EN_GraphsLinks('Property','Value',...) creates a new EN_GraphsLinks or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before NODEgraph_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to EN_GraphsLinks_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help EN_GraphsLinks

% Last Modified by GUIDE v2.5 26-Oct-2015 13:36:43

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @EN_GraphsLinks_OpeningFcn, ...
                   'gui_OutputFcn',  @EN_GraphsLinks_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before EN_GraphsLinks is made visible.
function EN_GraphsLinks_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to EN_GraphsLinks (see VARARGIN)

% Choose default command line output for EN_GraphsLinks
...
% UIWAIT makes EN_GraphsLinks wait for user response (see UIRESUME)
% uiwait(handles.figure1);
set(handles.figure1,'name','Graphs for Links');
...
set(handles.searchLinks,'String',ids{1});
...
set(handles.figure1,'Position',[40 7 188.5 40]);
set(handles.axes1,'position',[55 1.8 70 28]);

yesokpushbutton_Callback(hObject, eventdata, handles);


% --- Outputs from this function are returned to the command line.
function varargout = EN_GraphsLinks_OutputFcn(hObject, eventdata, handles)
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    set(handles.axes1,'position',[59.8 3.692 76.6 26.154]);
    ...
    [units,duration,x,T,parameter,type_plot]=GraphLinks(handles,index);
    % clear table
    tr = {'' ''};
...
% --- Executes on selection change in linkgraphmenu.
function linkgraphmenu_Callback(hObject, eventdata, handles)
% hObject    handle to linkgraphmenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = get(hObject,'String') returns linkgraphmenu contents as cell array
%        contents{get(hObject,'Value')} returns selected item from linkgraphmenu
set(handles.loading,'visible','on');
pause(0.01);
yesokpushbutton_Callback(hObject, eventdata, handles)
...
% --- Executes during object creation, after setting all properties.
function linkgraphmenu_CreateFcn(hObject, eventdata, handles)
% hObject    handle to linkgraphmenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
hours = get(handles.listbox1,'Value');
hours = hours-1;

set(handles.edit2,'String',hours);
set(handles.loading,'visible','on');
pause(0.01);
yesokpushbutton_Callback(hObject, eventdata, handles)
...
% yesokpushbutton_Callback(hObject, hours, handles)
% --- Executes during object creation, after setting all properties.
...
function  [res,times]=GraphsL(index,parameter,times,res,duration,type_plot,units,handles)
...
axes(handles.axes1);
if cmp==1
    bar(handles.axes1,times/3600,res(:,index),'b');
elseif cmp==0
%     plot(handles.axes1,(T(find(mod(T,timestep)==0)))/timestep,x(find(mod(T,timestep)==0),index),'r');
    plot(handles.axes1,times/3600,res(:,index),'r');
end
n= ids{index};
str = [parameter,' ','for Link ',n];
title(handles.axes1,str);
ylabel(handles.axes1,units);
xlabel(handles.axes1,'Time(hours)');
grid on;
if duration
    xlim([0,duration]);
end
...
set(handles.searchLinks,'String',id);
...
function [units,duration,x,T,parameter,type_plot]=GraphLinks(handles,index)
    ...
    param = get(handles.linkgraphmenu,'Value');
    if a==1 
        type_plot='bar';
        set(handles.plot,'value',0);
    end
    ...
    if a==0 && b==0
        ...
        t = handles.ep.B.getLinkCount;
        if size(index)~=0
            units=handles.ep.B.getBinUnits;
            if index > t || index==0
                    uiwait(msgbox('Wrong link ID.','modal'));return;
            else         
                switch param 
                    case 1
                        parameter='Length';
                        times=handles.res.Time;
                        e=handles.ep.B.getLinkLength;
                        res=ones(length(handles.res.Time),index)*e(index);
                        units =sprintf('Length (%s)',units.BinLinkLengthsUnits);
                    case 2
                        parameter='Diameter';
                        times=handles.res.Time;
                        e=handles.ep.B.getLinkDiameter;
                        res=ones(length(handles.res.Time),index)*e(index);
                        units = sprintf('Diameter (%s)',units.BinLinkPipeDiameterUnits);
                    case 3 
                        parameter='Roughness';
                        times=handles.res.Time;
                        e=handles.ep.B.getLinkRoughnessCoeff;
                        res=ones(length(handles.res.Time),index)*e(index);
                        units = 'Roughness';
                    case 4
                        parameter='Bulk Coeff.';
                        times=handles.res.Time;
                        e=handles.ep.B.getLinkBulkReactionCoeff;
                        res=ones(length(handles.res.Time),index)*e(index);
                        units = 'Bulk Coeff.';
                    case 5 
                        parameter='Wall Coeff.';
                        times=handles.res.Time;
                        e=handles.ep.B.getLinkWallReactionCoeff;
                        res=ones(length(handles.res.Time),index)*e(index);
                        units = 'Wall Coeff.';
                    case 6
                        parameter='Flow';
                        times=handles.res.Time;
                        res=handles.res.Flow;
                        units = sprintf('Flow (%s)',units.BinLinkFlowUnits);
                    case 7
                        parameter='Velocity';
                        times=handles.res.Time;
                        res=handles.res.Velocity;
                        units = sprintf('Quality (%s)',units.BinQualityUnits(1:4));     
                    case 8
                        parameter='Headloss';
                        times=handles.res.Time;
                        res=handles.res.HeadLoss;
                        units = sprintf('Quality (%s)',units.BinLinkMinorLossCoeffUnits);     
                end
            end
        end
        [x,T]=GraphsL(index,parameter,times,res,duration,type_plot,units,handles);
    end




function searchLinks_Callback(hObject, eventdata, handles)
% hObject    handle to searchLinks (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of searchLinks as text
%        str2double(get(hObject,'String')) returns contents of searchLinks as a double
...
if index
    ...
    uiwait(msgbox('Wrong link ID.','modal'));
    index=get(handles.listbox,'Value');
    id=handles.ep.B.getLinkNameID(index);
    set(handles.searchLinks,'String',id);
end
...
% --- Executes during object creation, after setting all properties.
function searchLinks_CreateFcn(hObject, eventdata, handles)
% hObject    handle to searchLinks (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
handles.ep.msg=get(handles.ep.LoadText,'String');
handles.ep.msg=[handles.ep.msg;{'>>Close Graphs for Links.'}];
set(handles.ep.LoadText,'Value',length(handles.ep.msg)); 
set(handles.ep.LoadText,'String',handles.ep.msg);
